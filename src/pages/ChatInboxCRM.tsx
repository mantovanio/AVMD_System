...
type ClaraFeedbackRating = 'boa' | 'corrigir' | 'risco'
...
export default function ChatInboxCRM() {
  ...
  const [saveAsQuickReply, setSaveAsQuickReply] = useState(false)
  ...
  function toggleConversationSelection(itemId: string) {
  ...
  function handleConversationClick(itemId: string, event: React.MouseEvent<HTMLButtonElement>) {
    if (event.ctrlKey || event.metaKey) {
      toggleConversationSelection(itemId)
      return
    }
  ...
  function openClaraFeedback(message: CrmMessage, rating: ClaraFeedbackRating = 'corrigir') {
    setClaraFeedbackDraft({
      message,
      rating,
      correctedResponse: '',
      notes: '',
  ...
  async function saveClaraFeedback() {
    if (!selectedConversation || !profile?.id || !claraFeedbackDraft) return
    setSavingClaraFeedback(true)
    setActionError(null)
    try {
      const response = await fetch(getApiUrl('/chat/crm/clara-feedback'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          conversation_id: selectedConversation.id,
          message_id: claraFeedbackDraft.message.id,
          profile_id: profile.id,
          rating: claraFeedbackDraft.rating,
          original_message: claraFeedbackDraft.message.mensagem || '',
          corrected_response: claraFeedbackDraft.correctedResponse,
          notes: claraFeedbackDraft.notes,
          metadata: claraFeedbackDraft.message.metadata || {},
        }),
      })
      const data = await response.json().catch(() => null) as { ok?: boolean; error?: string } | null
      if (!response.ok || !data?.ok) throw new Error(data?.error || 'Nao foi possivel salvar a correcao da Clara.')
      setClaraFeedbackDraft(null)
    } catch (err) {
      setActionError(err instanceof Error ? err.message : String(err))
    } finally {
      setSavingClaraFeedback(false)
    }
  }
  ...
  async function handleSaveAsQuickReply() {
    if (!qrShortcut.trim() || !qrName.trim()) return
    setQrSaving(true)
    try {
      const res = await fetch(getApiUrl('/chat/quick-replies'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
                ...
                <div ref={inboxListRef} className="h-[calc(100%-73px)] overflow-y-auto p-3">
                  ...
                  {filteredConversations.map(item => (
                    <ConversationCard
                      ...
                      onClick={event => handleConversationClick(item.id, event)}
                ...
                <div className="flex min-h-0 min-w-0 flex-1 flex-col xl:border-r xl:border-slate-200">
                  <div className="shrink-0 border-b border-slate-200 px-4 py-3">
                    ...
                    <div ref={messagesViewportRef} className="min-h-0 flex-1 overflow-y-auto bg-slate-50 px-4 py-4">
                      {!humanModeActive && (
                        <div className="mb-3 flex items-center justify-between gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-amber-800">
                          <div className="flex min-w-0 items-center gap-3">
                          ...
                          <span className="hidden rounded-full bg-white/70 px-3 py-1 text-[11px] font-bold uppercase tracking-wide text-amber-700 md:inline-flex">
                            modo IA
                          </span>
                        </div>
                      )}
                    ...
                    {humanModeActive ? (
                      ...
                      <div className="flex items-end gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-3">
                        ...
                        {humanMessage.trim() ? (
                          <>
                            <button type="button" onClick={() => { setQrMessageText(humanMessage.trim()); setQrShortcut(''); setQrName(''); setQrCategory(''); setSaveAsQuickReply(true) }} title="Salvar como resposta rapida" className="inline-flex h-11 w-11 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 hover:bg-slate-50 hover:text-sky-600 disabled:opacity-50">
                              <Bookmark size={16} />
                            </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-[2px]">
          <div className="w-full max-w-2xl rounded-[28px] border border-slate-200 bg-white p-5 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.25em] text-sky-500">Aprendizado da Clara</p>
                <h3 className="mt-1 text-xl font-semibold text-slate-900">Corrigir resposta da Clara</h3>
                <p className="mt-1 text-sm text-slate-500">Esse registro entra na fila de melhoria do playbook, sem alterar a IA automaticamente.</p>
              </div>
              <button type="button" onClick={() => setClaraFeedbackDraft(null)} className="rounded-xl border border-slate-200 p-2 text-slate-500 hover:bg-slate-50">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="mt-4 space-y-4">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3">
                ...
                <p className="mt-2 whitespace-pre-wrap text-sm text-slate-800">{claraFeedbackDraft.message.mensagem || 'Mensagem sem texto'}</p>
              </div>
              ...
              <div className="grid gap-2 sm:grid-cols-3">
                {([
                  ...
                  <button
                    key={value}
                    type="button"
                    onClick={() => setClaraFeedbackDraft(prev => prev ? { ...prev, rating: value } : prev)}
                    className={`rounded-2xl border px-3 py-2 text-sm font-semibold transition ${
                      claraFeedbackDraft.rating === value
              ...
              <label className="block space-y-1">
                <span className="text-xs font-semibold uppercase tracking-wide text-slate-500">Resposta ideal</span>
                <textarea
                  value={claraFeedbackDraft.correctedResponse}
                  onChange={event => setClaraFeedbackDraft(prev => prev ? { ...prev, correctedResponse: event.target.value } : prev)}
                  rows={5}
                  placeholder="Escreva aqui como a Clara deveria responder nesse caso."
                  className="w-full resize-none rounded-2xl border border-slate-200 bg-white px-3 py-3 text-sm outline-none focus:border-sky-400"
                />
              </label>

              <label className="block space-y-1">
                <span className="text-xs font-semibold uppercase tracking-wide text-slate-500">Observação interna</span>
                <textarea
                  value={claraFeedbackDraft.notes}
                  onChange={event => setClaraFeedbackDraft(prev => prev ? { ...prev, notes: event.target.value } : prev)}
                  rows={3}
                  placeholder="Ex.: token A3 nunca deve ser tratado como celular; pedir erro da tela primeiro."
                  className="w-full resize-none rounded-2xl border border-slate-200 bg-white px-3 py-3 text-sm outline-none focus:border-sky-400"
                />
              </label>

              <div className="flex flex-wrap items-center justify-end gap-2">
                <button type="button" onClick={() => setClaraFeedbackDraft(null)} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
                  Cancelar
                </button>
                <button type="button" onClick={() => void saveClaraFeedback()} disabled={savingClaraFeedback} className="inline-flex items-center gap-2 rounded-xl bg-sky-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">
                  {savingClaraFeedback ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />}
                  Salvar aprendizado
                </button>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-base font-semibold text-slate-800">Salvar como Resposta Rapida</h4>
              <button onClick={() => setSaveAsQuickReply(false)} className="rounded-lg p-1 text-slate-400 hover:bg-slate-100"><X className="h-5 w-5" /></button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">Atalho (sem \)</label>
                <input value={qrShortcut} onChange={e => setQrShortcut(e.target.value.replace(/[^a-zA-Z0-9_-]/g, ''))} placeholder="ex: obrigado" className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-sky-400 font-mono" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">Nome</label>
                <input value={qrName} onChange={e => setQrName(e.target.value)} placeholder="ex: Agradecimento" className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-sky-400" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">Categoria</label>
                <select value={qrCategory} onChange={e => setQrCategory(e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-sky-400 bg-white">
              ...
              <div>
                ...
                <textarea value={qrMessageText} onChange={e => setQrMessageText(e.target.value)} rows={3} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-sky-400 resize-none" />
              </div>
            </div>
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setSaveAsQuickReply(false)} className="rounded-xl border border-slate-200 px-4 py-2 text-sm text-slate-600 hover:bg-slate-50">Cancelar</button>
              <button onClick={() => void handleSaveAsQuickReply()} disabled={qrSaving || !qrShortcut.trim() || !qrName.trim()} className="flex items-center gap-1 rounded-xl bg-sky-600 px-4 py-2 text-sm font-medium text-white hover:bg-sky-700 disabled:opacity-60">
                {qrSaving && <Loader2 className="h-4 w-4 animate-spin" />}
                Salvar
              </button>
            </div>
          </div>
            ...
            <div className="mt-3 rounded-2xl border border-sky-200 bg-white/75 px-3 py-2 text-xs text-slate-700 shadow-sm dark:border-sky-500/30 dark:bg-slate-950/60 dark:text-slate-200">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {onClaraFeedback && (
                  <button
                    type="button"
                    onClick={() => onClaraFeedback(message, claraAudit.risk === 'alto' ? 'risco' : 'corrigir')}
                    className="ml-auto rounded-full border border-sky-200 bg-white px-2.5 py-1 text-[11px] font-semibold text-sky-700 transition hover:bg-sky-50 dark:border-sky-500/30 dark:bg-slate-900 dark:text-sky-200"
                  >
                    Corrigir resposta
                  </button>
                )}
