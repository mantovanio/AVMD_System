...
BEGIN
  ...
  IF v_phone = '' THEN
    -- Tentar extrair do conversation_id
    IF NEW.conversation_id LIKE '%@s.whatsapp.net' THEN
      v_phone := split_part(NEW.conversation_id, '@', 1);
    ELSE
  ...
  -- Upsert conversa
  INSERT INTO crm_chat_conversations (document_key, telefone, whatsapp_instance, ultima_mensagem, ultima_mensagem_direcao, ultima_interacao_em)
  VALUES (v_phone, v_phone, v_instance, v_content, v_direction, NEW.created_at)
