-- Fix: Evolution API tambem usa @hosted.lid para JIDs do WhatsApp
-- Alem disso, eventos com source='evolution' nunca sao email
...
-- Corrigir conversas existentes com @hosted.lid classificadas como email
UPDATE crm_chat_conversations
SET
  fila = 'geral',
  cliente_nome = regexp_replace(COALESCE(cliente_nome, ''), '[@:].*$', ''),
  telefone = regexp_replace(document_key, '@.*$', ''),
  updated_at = NOW()
WHERE fila = 'email'
  AND (document_key LIKE '%@hosted.lid' OR document_key LIKE '%@lid');
