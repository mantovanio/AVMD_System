-- Garante extracao profunda de midia do payload da Evolution
-- para mensagens com audio, imagem, video e documento.
...
BEGIN
  ...
  v_media_payload := COALESCE(
    NEW.payload#>'{data,message,imageMessage}',
    NEW.payload#>'{data,message,videoMessage}',
    NEW.payload#>'{data,message,audioMessage}',
    NEW.payload#>'{data,message,documentMessage}',
    NEW.payload#>'{message,imageMessage}',
    NEW.payload#>'{message,videoMessage}',
    NEW.payload#>'{message,audioMessage}',
    NEW.payload#>'{message,documentMessage}',
    NEW.payload#>'{data,message}',
    NEW.payload->'message'
  );

  -- extrair metadata de media do payload em profundidade
  v_mime_type := COALESCE(
    NEW.payload->>'mimeType',
    NEW.payload->>'mime_type',
    NEW.payload#>>'{message,mimetype}',
    NEW.payload#>>'{data,message,mimeType}',
    NEW.payload#>>'{data,message,mimetype}',
    v_media_payload->>'mimetype',
    v_media_payload->>'mimeType',
    v_media_payload->>'mime_type',
    ''
  );
  v_file_name := COALESCE(
    NEW.payload->>'fileName',
    NEW.payload->>'title',
    NEW.payload->>'filename',
    NEW.payload#>>'{data,message,fileName}',
    NEW.payload#>>'{data,message,filename}',
    NEW.payload#>>'{data,message,title}',
    NEW.payload#>>'{data,message,caption}',
    v_media_payload->>'fileName',
    v_media_payload->>'filename',
    v_media_payload->>'title',
    v_media_payload->>'caption',
    ''
  );
  v_media_url := COALESCE(
    NEW.payload->>'mediaUrl',
    NEW.payload->>'url',
    NEW.payload->>'link',
    NEW.payload#>>'{data,message,mediaUrl}',
    NEW.payload#>>'{data,message,url}',
    NEW.payload#>>'{data,message,link}',
    NEW.payload#>>'{data,message,directPath}',
    v_media_payload->>'mediaUrl',
    v_media_payload->>'url',
    v_media_payload->>'link',
    v_media_payload->>'directPath',
    ''
  );
