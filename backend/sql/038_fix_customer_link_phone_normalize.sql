-- Corrige o vinculo automatico cliente<->conversa dentro da trigger
-- fn_sync_communication_event(): o bloco que acha o crm_customers
-- correspondente a mensagem recebida comparava telefone com
-- regexp_replace cru (so tirava caracteres nao-numericos, sem tirar o
-- DDI), diferente do indice unico (fn_normalize_phone_br, que remove o
-- DDI). Isso fazia o sistema "nao reconhecer" clientes ja cadastrados
-- quando o formato do telefone recebido variava (com/sem 55).
--
-- Recria a funcao a partir da versao 037 (baseline atual em producao),
-- mudando so o predicado de telefone desse bloco para usar a mesma
-- normalizacao canonica usada em todo o resto do sistema.
...
BEGIN
  ...
  -- Unica mudanca desta migracao em relacao a 037: o predicado de
  -- telefone agora compara os dois lados ja normalizados
  -- (fn_normalize_phone_br), em vez de regexp_replace cru. Assim um
  -- cliente ja cadastrado e reconhecido independente do DDI vir ou nao
  -- no evento recebido.
