-- Fix: Evolution API usa @lid para JIDs do WhatsApp
-- O trigger estava classificando @lid como email porque nao excluia esse padrao
...
BEGIN
  ...
  v_is_email := NEW.source = 'email' OR (v_phone LIKE '%@%' AND v_phone NOT LIKE '%@s.whatsapp.net' AND v_phone NOT LIKE '%@g.us' AND v_phone NOT LIKE '%@broadcast%' AND v_phone NOT LIKE '%@lid');
...
-- Corrigir conversas existentes classificadas erroneamente como email por causa do @lid
UPDATE crm_chat_conversations
SET
  fila = 'geral',
  cliente_nome = regexp_replace(cliente_nome, '[@:].*$', ''),
  telefone = regexp_replace(document_key, '@.*$', ''),
  updated_at = NOW()
WHERE fila = 'email'
  AND document_key LIKE '%@lid';
