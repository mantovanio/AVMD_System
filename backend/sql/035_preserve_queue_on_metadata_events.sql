-- Preserva a fila/instancia da conversa quando eventos administrativos
-- (ex.: contact_updated) nao trazem sinal real de roteamento.
...
BEGIN
  ...
  v_fila := CASE
    ...
    WHEN v_has_content THEN 'atendimento'
    ELSE NULL
  END;
...
-- Reclassifica conversas que foram derrubadas para atendimento por eventos administrativos.
UPDATE crm_chat_conversations c
       ...
       whatsapp_instance = COALESCE(
         ...
         NULLIF((
           SELECT COALESCE(ce.payload->>'instance_name', ce.payload->>'instanceName', '')
            ...
            WHERE (
                    ce.contact = regexp_replace(COALESCE(c.telefone, c.document_key, ''), '[^0-9]', '', 'g')
                 OR ce.conversation_id = c.document_key || '@s.whatsapp.net'
                 OR ce.conversation_id = c.document_key
                  )
              AND (
 ...
 WHERE EXISTS (
   ...
   AND COALESCE(c.fila, 'atendimento') <> 'renovacao';
