-- Corrige v_telefone no trigger para usar o phone limpo (sem @s.whatsapp.net)
-- e limpa registros existentes que ficaram com o JID completo.
...
BEGIN
  ...
  -- telefone vem DEPOIS da limpeza do v_phone (diferente do 019 original)
...
-- Limpar telefone que ficou com o JID completo (ex: 5512988188337@s.whatsapp.net)
UPDATE crm_chat_conversations
SET telefone = regexp_replace(telefone, '@.*$', ''), updated_at = NOW()
WHERE telefone LIKE '%@%';
