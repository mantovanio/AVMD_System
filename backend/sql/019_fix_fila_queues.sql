-- Corrige trigger para usar 'atendimento' e 'renovacao' em vez de 'geral'
-- WhatsApp/Evolution → 'atendimento' (padrao)
-- WhatsApp com instance/canal 'renovacao' → 'renovacao'
-- Email com kanban_status agendado/cancelou → 'agendamento'
-- Email geral → 'email'
...
BEGIN
  ...
  -- Ignora eventos evolution sem conteudo real (status update, @lid sem remoteJid)
...
-- Corrigir conversas existentes: 'geral' → 'atendimento'
UPDATE crm_chat_conversations
SET fila = 'atendimento', updated_at = NOW()
WHERE fila = 'geral';

-- Remover conversas fantasmas criadas por messages.update com @lid
-- (document_key puramente numerico >=14 digitos, sem instancia, sem evento real)
DELETE FROM crm_chat_messages
WHERE conversation_id IN (
  SELECT c.id FROM crm_chat_conversations c
  WHERE (c.whatsapp_instance IS NULL OR c.whatsapp_instance = '')
    AND LENGTH(c.document_key) >= 14
    AND c.document_key ~ '^[0-9]+$'
);
DELETE FROM crm_chat_conversations c
WHERE (c.whatsapp_instance IS NULL OR c.whatsapp_instance = '')
  AND LENGTH(c.document_key) >= 14
  AND c.document_key ~ '^[0-9]+$';
