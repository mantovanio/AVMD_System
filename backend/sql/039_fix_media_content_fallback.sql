-- Garante que mensagens de mídia/anexo sem legenda não entrem com
-- mensagem vazia no CRM chat.
--
-- A trigger atual em fn_sync_communication_event() prioriza content/body,
-- mas alguns eventos da Evolution chegam apenas com mimeType, mediaUrl
-- e fileName. Nesse caso, salvamos um rótulo humano legível para que o
-- histórico e a última mensagem não fiquem em branco.
...
BEGIN
  ...
  v_file_name := COALESCE(NULLIF(NEW.payload->>'fileName', ''), NULLIF(NEW.payload->>'file_name', ''));
  v_mime_type := COALESCE(NULLIF(NEW.payload->>'mimeType', ''), NULLIF(NEW.payload->>'mime_type', ''));
  ...
  v_content := COALESCE(
    ...
    NEW.payload->>'body',
    CASE
      WHEN v_file_name IS NOT NULL AND v_file_name <> '' THEN 'Arquivo: ' || v_file_name
      WHEN v_mime_type LIKE 'image/%' THEN 'Imagem'
      WHEN v_mime_type LIKE 'video/%' THEN 'Vídeo'
      WHEN v_mime_type LIKE 'audio/%' THEN 'Áudio'
      WHEN v_mime_type LIKE 'application/%' THEN 'Documento'
      ELSE ''
    END
  );
