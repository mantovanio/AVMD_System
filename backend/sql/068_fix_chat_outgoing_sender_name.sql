-- Corrige regressao no chat CRM: mensagens enviadas pelo operador nao
-- podem sobrescrever o nome do cliente com o nome do remetente.
--
-- A trigger fn_sync_communication_event() vinha usando sender_name /
-- pushName como fallback para cliente_nome mesmo quando fromMe = true.
-- Isso fazia conversas novas aparecerem como "Operador" e confundia o
-- inbox com IA.
...
BEGIN
  ...
  v_sender_type := CASE WHEN v_is_from_me THEN 'humano' ELSE 'cliente' END;
...
WITH latest_conversation_name AS (
  SELECT
    c.id,
    (
      SELECT m.sender_name
       ...
       WHERE m.conversation_id = c.id
         ...
         AND NULLIF(btrim(coalesce(m.sender_name, '')), '') IS NOT NULL
...
UPDATE crm_chat_conversations c
   SET cliente_nome = CASE
         WHEN latest_conversation_name.incoming_name IS NULL OR btrim(latest_conversation_name.incoming_name) = '' THEN c.cliente_nome
         ELSE latest_conversation_name.incoming_name
       END,
       updated_at = NOW()
  FROM latest_conversation_name
 WHERE c.id = latest_conversation_name.id
   AND (
     c.cliente_nome IS NULL
     OR btrim(c.cliente_nome) = ''
     OR lower(btrim(c.cliente_nome)) IN ('operador', 'humano', 'ia clara')
   );
