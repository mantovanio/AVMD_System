...
const server = createServer(async (req, res) => {
  try {
    ...
    const handledCatalog = await handleCatalogRoutes(req, res, catalogRepository, renovacaoRepository, db, corsOrigin, config, profileRepository)
    ...
    await handleCheckoutRoutes(req, res, service, corsOrigin, checkoutPaymentService)
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Erro interno do servidor.'
    console.error(`[HTTP ${req.method ?? 'UNKNOWN'} ${req.url ?? '/'}] ${message}`, error)
    writeJson(res, 500, { ok: false, error: message }, resolveCorsOrigin(typeof req.headers.origin === 'string' ? req.headers.origin : undefined, config.corsOrigin))
  }
})
